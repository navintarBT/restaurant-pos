...
// Dedicated "create food group(s)" screen — kept separate from
// ManageFoodGroups.tsx (which only lists/deletes what's already saved),
// mirroring CreateZone.tsx's split. One save here can add several groups at
// once: each row is its own name, "+" adds another blank row.
const CreateFoodGroup: React.FC = () => {
  ...
  useEffect(() => {
    if (!shopId) return;
    setLoading(true);
    getFoodGroups(shopId).finally(() => setLoading(false));
  }, [shopId]);
  ...
  async function handleSubmit() {
    ...
    for (const n of names) {
      if (seen.has(n)) {
        showResult(false, `ກຸ່ມອາຫານ "${n}" ຊໍ້າກັນໃນລາຍການທີ່ພິມ`);
        return;
      }
      seen.add(n);
    }
    ...
    try {
      ...
      if (collision) {
        showResult(false, `ກຸ່ມອາຫານ "${collision}" ມີຢູ່ແລ້ວ`);
        return;
      }
      await setFoodGroups(shopId, [...current, ...names]);
      showResult(true, names.length > 1 ? `ສ້າງ ${names.length} ກຸ່ມອາຫານສຳເລັດ` : "ສ້າງກຸ່ມອາຫານສຳເລັດ");
    } catch {
      showResult(false, "ສ້າງກຸ່ມອາຫານບໍ່ສຳເລັດ, ລອງໃໝ່");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>ສ້າງກຸ່ມອາຫານໃໝ່</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink="/tabs/manage-food-groups" routerDirection="back">
          ...
          <div style={{ padding: "16px 16px 32px" }}>
            <div style={{ border: "1px solid var(--app-border)", borderRadius: 12, padding: "14px 16px" }}>
              ...
              <p style={{ margin: "0 0 12px", fontSize: "0.76rem", color: "var(--app-text-secondary)" }}>
                ເຊັ່ນ: ກຸ່ມເຄື່ອງດື່ມ, ກຸ່ມອາຫານ. ເພີ່ມໄດ້ຫຼາຍກຸ່ມພ້ອມກັນ ກົດ "+" ເພື່ອເພີ່ມແຖວ
              </p>
              {rows.map((row) => (
                <div key={row.id} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <IonInput
                    placeholder="ເຊັ່ນ: ກຸ່ມເຄື່ອງດື່ມ" value={row.value}
      ...
      <IonFooter>
        <div style={{ padding: "12px 16px 28px", background: "var(--ion-item-background, #fff)", borderTop: "1px solid var(--app-border)", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <IonButton
            ...
            routerLink="/tabs/manage-food-groups" routerDirection="back"
            style={{ minHeight: 50, "--border-radius": "14px", margin: 0 }}
          >
            ຍົກເລີກ
          </IonButton>
        ...
        buttons={[
          {
            ...
            handler: () => {
              setResultOpen(false);
              if (resultSuccess) history.push("/tabs/manage-food-groups");
            },
          },
        ]}
