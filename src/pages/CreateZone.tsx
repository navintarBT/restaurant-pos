...
import { getTableZones, setTableZones } from "../data/shopRepository";

let nextRowId = 1;

// Dedicated "create zone(s)" screen — kept separate from ManageZones.tsx
// (which only lists/deletes what's already saved), mirroring TableForm.tsx's
// split. Unlike tables, one save here can add several zones at once: each
// row is its own name, "+" adds another blank row, and Save writes all of
// them together.
const CreateZone: React.FC = () => {
  ...
  useEffect(() => {
    if (!shopId) return;
    setLoading(true);
    getTableZones(shopId).finally(() => setLoading(false));
  }, [shopId]);
  ...
  async function handleSubmit() {
    ...
    for (const n of names) {
      if (seen.has(n)) {
        showResult(false, `ຊື່ໂຊນ "${n}" ຊໍ້າກັນໃນລາຍການທີ່ພິມ`);
        return;
      }
      seen.add(n);
    }
    ...
    try {
      ...
      const currentZones = await getTableZones(shopId);
      const collision = names.find((n) => currentZones.includes(n));
      if (collision) {
        showResult(false, `ໂຊນ "${collision}" ມີຢູ່ແລ້ວ`);
        return;
      }
      await setTableZones(shopId, [...currentZones, ...names]);
      showResult(true, names.length > 1 ? `ສ້າງ ${names.length} ໂຊນສຳເລັດ` : "ສ້າງໂຊນສຳເລັດ");
    } catch {
      showResult(false, "ສ້າງໂຊນບໍ່ສຳເລັດ, ລອງໃໝ່");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>ສ້າງໂຊນໃໝ່</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink="/tabs/manage-zones" routerDirection="back">
          ...
          <div style={{ padding: "16px 16px 32px" }}>
            <div style={{ border: "1px solid var(--app-border)", borderRadius: 12, padding: "14px 16px" }}>
              ...
              <p style={{ margin: "0 0 12px", fontSize: "0.76rem", color: "var(--app-text-secondary)" }}>
                ເພີ່ມໄດ້ຫຼາຍໂຊນພ້ອມກັນ ກົດ "+" ເພື່ອເພີ່ມແຖວ
              </p>
              {rows.map((row) => (
                <div key={row.id} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <IonInput
                    placeholder="ເຊັ່ນ: ໃນຮ້ານ" value={row.value}
      ...
      <IonFooter>
        <div style={{ padding: "12px 16px 28px", background: "var(--ion-item-background, #fff)", borderTop: "1px solid var(--app-border)", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <IonButton
            ...
            routerLink="/tabs/manage-zones" routerDirection="back"
            style={{ minHeight: 50, "--border-radius": "14px", margin: 0 }}
          >
            ຍົກເລີກ
          </IonButton>
        ...
        buttons={[
          {
            ...
            handler: () => {
              setResultOpen(false);
              if (resultSuccess) history.push("/tabs/manage-zones");
            },
          },
        ]}
