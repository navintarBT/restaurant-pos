...
import { getSizes, setSizes } from "../data/shopRepository";
import EmptyState from "../components/EmptyState";

// Size CREATION lives on its own page (CreateSize.tsx, reached via the "+"
// button below) and supports adding several sizes in one save — this page
// is just the list. Deleting a size applies immediately. Mirrors
// ManageUnits.tsx's structure exactly (same search/sort/limit pattern).
const ManageSizes: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setSizesState(await getSizes(shopId));
    } finally {
      setLoading(false);
    }
  }, [shopId]);
  ...
  // Sizes are appended to the end of the array when created (see
  // CreateSize.tsx), so array order already IS creation order — "oldest" is
  ...
  async function confirmDelete() {
    ...
    setError(null);
    try {
      const current = await getSizes(shopId);
      const updated = current.filter((s) => s !== deleteTarget);
      await setSizes(shopId, updated);
      setSizesState(updated);
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>ຈັດການຂະໜາດ</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink="/tabs/create-size">
          ...
          <div style={{ padding: "12px 16px 28px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--app-text-secondary)" }}>
                ສະແດງ {processed.length} ຈາກ {sizes.length} ຂະໜາດ
              </p>
              <IonButton routerLink="/tabs/create-size" size="small" style={{ "--border-radius": "10px", margin: 0 }}>
            ...
            {sizes.length === 0 && <EmptyState icon="📏" title="ຍັງບໍ່ມີຂະໜາດ" subtitle="ກົດ 'ສ້າງຂະໜາດ' ເພື່ອເລີ່ມ" />}
            {sizes.length > 0 && processed.length === 0 && <EmptyState icon="🔍" title="ບໍ່ພົບຂະໜາດທີ່ຄົ້ນຫາ" />}
            {processed.map((s) => (
              <div key={s} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--app-border)" }}>
                <span style={{ fontWeight: 700, fontSize: "0.92rem" }}>{s}</span>
                <button
      ...
      <IonAlert
        isOpen={!!deleteTarget}
        header="ລຶບຂະໜາດ"
        message={`ຕ້ອງການລຶບ "${deleteTarget}" ແມ່ນບໍ່?`}
        buttons={[
