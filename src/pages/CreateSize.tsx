...
// Dedicated "create size(s)" screen — kept separate from ManageSizes.tsx
// (which only lists/deletes what's already saved), mirroring CreateUnit.tsx's
// split. One save here can add several sizes at once: each row is its own
...
const CreateSize: React.FC = () => {
  ...
  useEffect(() => {
    if (!shopId) return;
    setLoading(true);
    getSizes(shopId).finally(() => setLoading(false));
  }, [shopId]);
  ...
  async function handleSubmit() {
    ...
    for (const n of names) {
      if (seen.has(n)) {
        showResult(false, `ຂະໜາດ "${n}" ຊໍ້າກັນໃນລາຍການທີ່ພິມ`);
        return;
      }
      seen.add(n);
    }
    ...
    try {
      ...
      if (collision) {
        showResult(false, `ຂະໜາດ "${collision}" ມີຢູ່ແລ້ວ`);
        return;
      }
      await setSizes(shopId, [...current, ...names]);
      showResult(true, names.length > 1 ? `ສ້າງ ${names.length} ຂະໜາດສຳເລັດ` : "ສ້າງຂະໜາດສຳເລັດ");
    } catch {
      showResult(false, "ສ້າງຂະໜາດບໍ່ສຳເລັດ, ລອງໃໝ່");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>ສ້າງຂະໜາດໃໝ່</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink="/tabs/manage-sizes" routerDirection="back">
          ...
          <div style={{ padding: "16px 16px 32px" }}>
            <div style={{ border: "1px solid var(--app-border)", borderRadius: 12, padding: "14px 16px" }}>
              ...
              <p style={{ margin: "0 0 12px", fontSize: "0.76rem", color: "var(--app-text-secondary)" }}>
                ເຊັ່ນ: ນ້ອຍ, ກາງ, ໃຫຍ່. ເພີ່ມໄດ້ຫຼາຍຂະໜາດພ້ອມກັນ ກົດ "+" ເພື່ອເພີ່ມແຖວ
              </p>
              {rows.map((row) => (
                <div key={row.id} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <IonInput
                    placeholder="ເຊັ່ນ: ກາງ" value={row.value}
      ...
      <IonFooter>
        <div style={{ padding: "12px 16px 28px", background: "var(--ion-item-background, #fff)", borderTop: "1px solid var(--app-border)", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <IonButton
            ...
            routerLink="/tabs/manage-sizes" routerDirection="back"
            style={{ minHeight: 50, "--border-radius": "14px", margin: 0 }}
          >
            ຍົກເລີກ
          </IonButton>
        ...
        buttons={[
          {
            ...
            handler: () => {
              setResultOpen(false);
              if (resultSuccess) history.push("/tabs/manage-sizes");
            },
          },
        ]}
