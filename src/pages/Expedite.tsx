...
const Expedite: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToOpenOrders(shopId, ["ready"], (newOrders) => {
  ...
  async function markServed(order: Sale) {
    if (!shopId) return;
    setBusyId(order.id);
    try {
      await advanceOrderStatus(shopId, order.id, "served");
  ...
  return (
    <IonPage>
      ...
      <IonContent>
        ...
        {!loading && orders.length === 0 && <EmptyState icon="🍽️" title="ບໍ່ມີອາຫານພ້ອມເສີບ" />}
        ...
        <div style={{ padding: "12px 16px 28px", display: "grid", gap: 12 }}>
          {orders.map((order) => (
            <div
              key={order.id}
              style={{
                background: "var(--app-success-surface)", borderRadius: 16, padding: "14px 16px",
                boxShadow: "0 2px 10px rgba(0,0,0,0.07)", border: "1.5px solid #86efac",
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <span style={{
                  ...
                  ພ້ອມແລ້ວ · {timeAgo(order.createdAt)}
                </span>
              </div>
