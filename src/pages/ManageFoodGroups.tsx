...
import { getFoodGroups, setFoodGroups } from "../data/shopRepository";
import EmptyState from "../components/EmptyState";

// Food-group CREATION lives on its own page (CreateFoodGroup.tsx, reached
// via the "+" button below) and supports adding several groups in one save
// — this page is just the list. Deleting a group applies immediately.
// Mirrors ManageZones.tsx's structure exactly. Food groups sit ONE LEVEL
// ABOVE categories (shops/{shopId}/categories) — each category can be
// tagged with the food group it belongs to, from ManageCategories.tsx.
const ManageFoodGroups: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setGroups(await getFoodGroups(shopId));
    } finally {
      setLoading(false);
    }
  }, [shopId]);
  ...
  // Groups are appended to the end of the array when created (see
  // CreateFoodGroup.tsx), so array order already IS creation order —
  // "oldest" is just the array as-is, "newest" is that reversed.
  ...
  async function confirmDelete() {
    ...
    setError(null);
    try {
      const current = await getFoodGroups(shopId);
      const updated = current.filter((g) => g !== deleteTarget);
      await setFoodGroups(shopId, updated);
      setGroups(updated);
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>ຈັດການກຸ່ມອາຫານ</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink="/tabs/create-food-group">
          ...
          <div style={{ padding: "12px 16px 28px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--app-text-secondary)" }}>
                ສະແດງ {processed.length} ຈາກ {groups.length} ກຸ່ມ
              </p>
              <IonButton routerLink="/tabs/create-food-group" size="small" style={{ "--border-radius": "10px", margin: 0 }}>
            ...
            {groups.length === 0 && <EmptyState icon="🍱" title="ຍັງບໍ່ມີກຸ່ມອາຫານ" subtitle="ກົດ 'ສ້າງກຸ່ມອາຫານ' ເພື່ອເລີ່ມ" />}
            {groups.length > 0 && processed.length === 0 && <EmptyState icon="🔍" title="ບໍ່ພົບກຸ່ມທີ່ຄົ້ນຫາ" />}
            {processed.map((g) => (
              <div key={g} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--app-border)" }}>
                <span style={{ fontWeight: 700, fontSize: "0.92rem" }}>{g}</span>
                <button
      ...
      <IonAlert
        isOpen={!!deleteTarget}
        header="ລຶບກຸ່ມອາຫານ"
        message={`ຕ້ອງການລຶບ "${deleteTarget}" ແມ່ນບໍ່? ໝວດໝູ່ທີ່ໃຊ້ກຸ່ມນີ້ຈະບໍ່ມີກຸ່ມ`}
        buttons={[
...
export default ManageFoodGroups;
