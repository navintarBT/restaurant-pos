...
// Unit CREATION lives on its own page (CreateUnit.tsx, reached via the "+"
// button below) and supports adding several units in one save — this page
// is just the list. Deleting a unit applies immediately. Mirrors
...
const ManageUnits: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setUnitsState(await getUnits(shopId));
    } finally {
      setLoading(false);
    }
  }, [shopId]);
  ...
  // Units are appended to the end of the array when created (see
  // CreateUnit.tsx), so array order already IS creation order — "oldest" is
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const updated = current.filter((u) => u !== deleteTarget);
      await setUnits(shopId, updated);
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>ຈັດການຫົວໜ່ວຍ</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink="/tabs/create-unit">
          ...
          <div style={{ padding: "12px 16px 28px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <p style={{ margin: 0, fontSize: "0.8rem", color: "var(--app-text-secondary)" }}>
                ສະແດງ {processed.length} ຈາກ {units.length} ຫົວໜ່ວຍ
              </p>
              <IonButton routerLink="/tabs/create-unit" size="small" style={{ "--border-radius": "10px", margin: 0 }}>
            ...
            {units.length === 0 && <EmptyState icon="🧊" title="ຍັງບໍ່ມີຫົວໜ່ວຍ" subtitle="ກົດ 'ສ້າງຫົວໜ່ວຍ' ເພື່ອເລີ່ມ" />}
            {units.length > 0 && processed.length === 0 && <EmptyState icon="🔍" title="ບໍ່ພົບຫົວໜ່ວຍທີ່ຄົ້ນຫາ" />}
            {processed.map((u) => (
              <div key={u} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid var(--app-border)" }}>
                <span style={{ fontWeight: 700, fontSize: "0.92rem" }}>{u}</span>
                <button
